...
def failover_config(tmp_path, cooldown_s: float = 30.0) -> Config:
    return Config(
        ...
        profiles=[
            ModelProfile(
                ...
                fallbacks=[FallbackTarget(model="fallback-model")],
            )
        ],
    )
...
def test_cooldown_routes_straight_to_fallback_on_next_request(tmp_path):
    ...
    # Primary is cooling down, so the very first call this request makes is the fallback.
...
def test_recovered_primary_is_used_again_after_cooldown(tmp_path):
    ...
    with TestClient(app) as client, respx.mock:
        ...
        # Fast-forward past the cooldown and make the primary healthy again.
        app.state.ctx.health._now = lambda: 10_000_000.0
        route.reset()
